...
export default function Portfolio() {
  ...
  return (
    <section id="portfolio" className="py-24 bg-white dark:bg-slate-900/50 transition-colors overflow-hidden">
            ...
            {projects.map((project, index) => (
              ...
              >
                <div className="bento-card group h-full">
                  <div className="relative aspect-video rounded-[2rem] overflow-hidden mb-8 border border-slate-100 dark:border-slate-800 shadow-inner">
                    <img 
                      src={project.image} 
                      alt={project.title}
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-8">
                      <button className="bg-white text-slate-900 px-6 py-3 rounded-2xl font-black text-sm flex items-center gap-2 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 hover:bg-blue-50">
                  ...
                  <p className="text-slate-600 dark:text-slate-400 leading-relaxed text-base sm:text-lg flex-grow font-medium">
                    {project.desc}
                  </p>
                </div>
              </motion.div>
